...
| 2026-05-28 | `feat/design-system-v2` | 23 | +957 | -661 | 3 | Brand swap to the new Zarco Design System (May 2026 handoff): magenta accent + Hanken Grotesk + DM Serif italic + JetBrains Mono + corner-notch Z mark. First pass landed on paper-first per the bundle, but in side-by-side test the dark-navy-on-amber tracker UX read better — overdue chips and hot deals didn't pop against a bright background. Final shape inverts the tokens: ink surfaces, off-white text, magenta still as the single accent. Same palette, mode-agnostic recipes — every `.btn` / `.input` / `.tbl` / `.nav-item` / `.chip` / `.dot` recipe flips cleanly by swapping which named token (`--paper` vs `--ink`) is the surface vs the foreground. Legacy var names (`--bg`, `--panel-2`, `--amber`, `--surface-3`, `--hairline`) re-bound, so no per-component edits needed. Status colors brightened for dark legibility (`--success` #4FBA84, `--warning` #E5A03D, `--danger` #E85870, `--info` #5B9BFF) with centralized `--*-wash` / `--*-edge` translucent variants for chips. `ZarcoMark` uses `var(--ink)`/`var(--paper)` for the tile + glyph, so it auto-inverts between modes; magenta corner notch is mode-agnostic. SVG favicon at `src/app/icon.svg`. Deal stages now use a temperature ramp (lead gray → qualified blue → proposal amber → negotiation magenta → won green / lost red) so the pipeline-by-stage bar reads at a glance — mirrored in both kanban + dashboard. Quote PDF stays on paper (customer artifact, never a UI surface) with the corner-notch Z mark + magenta accent rule. The `.screen` grain overlay is removed (was a dark-UI mannerism made redundant by the cleaner tokens). |
